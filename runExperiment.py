...
# Wrapper for calling simulator collision check since it doesn't return consistent argument type
...
if __name__ == "__main__":
    ...
    parser.add_argument("-nmp","--narrowmodelpath", type=str, metavar=('path'), default='~/sim/blimpswarm/models/blimpNarrowSensor-Cameron.ttm',
                help="Specify path to the narrow model. Default is '~/sim/wall-climb-simulation/blimpNarrowSensor.ttm'. \
                    Can also accept 'blimpNarrowSensor.ttm' if running from within the correct dir.")
    parser.add_argument("-wmp","--widemodelpath", type=str, metavar=('path'), default='~/sim/blimpswarm/models/blimpNarrowSensor-Cameron.ttm',
                help="Specify path to the narrow model. Default is '~/sim/wall-climb-simulation/blimpWideSensor.ttm'. \
                    Can also accept 'blimpWideSensor.ttm' if running from within the correct dir.")
    parser.add_argument("-sp","--scenepath", type=str, metavar=('path'), default='~/sim/blimpFlow/scenes/wallClimbEnv.ttt',
                help="Specify path to the simulation scene. Default is '~/sim/wall-climb-simulation/wallClimbEnv.ttt'. \
    ...
    parser.add_argument("-odp","--outputdirectorypath", type=str, metavar=('path'), default='~/sim/wall-climb-simulation/data',
                help="Specify path to the output directory for the summary file to be written within. Default is '~/sim/wall-climb-simulation/data")
    ...
    try:
        assert os.path.isdir(outputDirectoryPath)
    except:
        print('The output directory, [' + outputDirectoryPath + '] is not a directory or does not exist.\nExiting...')
        exit(1)
